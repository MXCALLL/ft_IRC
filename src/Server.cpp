...
void Server::run(Bot &IRCBot) {

  struct pollfd BotPfd;
  BotPfd.fd = IRCBot.getFd();
  BotPfd.events = POLLIN;
  BotPfd.revents = 0;
  Fd.push_back(BotPfd);
...
void Server::AcceptClient(void) {
  ...
  std::cout << "[IRCSERV]: New Connection from "
            << inet_ntoa(clientAddr.sin_addr) << " on fd " << clientFd
            << std::endl;
}
...
void Server::HandleCommand(std::string cmd, int fd) {
  ...
  while (!cmd.empty() &&
         (cmd[cmd.size() - 1] == '\r' || cmd[cmd.size() - 1] == '\n'))
  ...
  if (cmd.size() > 510) {
    ...
    SendReply(fd, ":" + std::string(SERVER_NAME) +
  ...
  if (!isPrintable(param)) {
    SendReply(fd, ":" + std::string(SERVER_NAME) +
                      " 400 * :Non-printable characters not allowed\r\n");
    return;
  }
  ...
  {
    if (param.find("LS") != std::string::npos)
      SendReply(fd, ":" + std::string(SERVER_NAME) + " CAP * LS :There are no modern features supported on this IRC Server\r\n");
  }
  else if (command == "PING")
    SendReply(fd, ":" + std::string(SERVER_NAME) + " PONG " + std::string(SERVER_NAME) + " :" + param + "\r\n");
